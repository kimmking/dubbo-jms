...
public class JmsExporter<T> extends AbstractExporter<T> {
	...
	// use the given connection and queue to subscribe invocation and execute for a result, 
	// then send the result back to replyTo queue
	public JmsExporter(final Invoker<T> invoker,final QueueConnectionFactory queueConnectionFactory,final Queue queue) {
		super(invoker);
		try {
